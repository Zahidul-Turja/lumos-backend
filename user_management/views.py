    ...
    try:
        ...
        if is_signup:
            user, created = User.objects.get_or_create(
                email=email,
                defaults={"username": email.split("@")[0], "is_email_verified": False},
            )
        else:
        ...
        subject = "Sign up for your account" if is_signup else "Sign in to your account"
        message = f"""
        Hello!
...
def google_auth(request):
    ...
    try:
        ...
        # Update user info if not created but google_id is missing
...
def logout(request):
    ...
    try:
        ...
        UserSession.objects.filter(
            user=request.user, session_key=request.session.session_key or "api_session"
